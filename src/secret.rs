...
use crate::{Result, get_k8s_env, check_secret};

#[tokio::main]
pub async fn process(namespace: &str, with_encoded: bool) -> Result<()> {
    check_secret()?;
    ...
    api.list(&ListParams::default())
        ...
        .into_iter()
        .filter(|x| {
            x.type_.as_ref().unwrap() == "Opaque"
        })
        .for_each(|s| { print_secret(s, with_encoded); });
    Ok(())
}
...
fn print_secret(s: Secret, with_encoded: bool) -> Option<()> {
    ...
    if let (Some(name), Some(data)) = (s.metadata.name, s.data) {
        ...
        data.iter().for_each(|(key, value)| {
            ...
            let secret = Decoded { key, decoded, base64: base64.as_str(), with_encoded } ;
            println!("{}", secret);
        })
    }
    println!();
...
impl<'a> Display for Decoded<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if self.with_encoded {
            write!(f, "      {0:_<50} {1:_<70} {2}", self.key, self.decoded, self.base64)            
        } else {
