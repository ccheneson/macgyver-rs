...
impl<'a> ContainerStatusOps<'a> {
    ...
    fn status(&self) -> ColoredString {
        ...
        if let Some(container_state) = &status.state {
            match (&container_state.running, &container_state.terminated, &container_state.waiting) {
                //Running
                (Some(_), _, _) => "Running".to_string().green(),
                //Terminated
                (_, Some(terminated), _) =>  {
                    let reason = terminated.reason.as_ref().unwrap_or(&"N/A".to_string()).yellow();
...
#[tokio::main]
pub  async fn process(namespace: &str,with_pod: bool) -> Result<()> {
    let context = get_k8s_env()?;
    let client = Client::try_default().await?;
    let api: Api<Pod> = Api::namespaced(client, namespace);

    let headers = PrintArguments {
        image: "IMAGE".to_string(), status : "STATUS".white().to_string(), 
        restart_count: "RESTART".white().to_string(), name: "NAME".to_string()
    };


    let infos : Vec<Option<PrintArguments>> = api.list(&ListParams::default())
                                                .await?
                                                .into_iter()
...
fn print_result(context: &str, info: &[Option<PrintArguments>], with_pod: bool) {
    ...
    info.into_iter()
        ...
        .for_each(|i| 
            if with_pod {
                println!("{0: <105} {1: <25} {2: <18} {3: <30}", i.image, i.status, i.restart_count, i.name)
            } else {
                println!("{0: <105} {1: <25} {2: <18}", i.image, i.status, i.restart_count)
            }
        );
}
...
fn collect_info(p: Pod) -> Option<PrintArguments> {
    ...
    let images : Vec<PodInfo> = containers.into_iter()
    ...
    for image in images {
        if let Some(container_status) = &status.container_statuses {
            let maybe_image_status = container_status.into_iter()
