...
impl Display for PrintArguments {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "{0: <100} {1: <20} {2: <20} {3: <20} {4: <20}", self.pod , self.limit_cpu, self.limit_memory, self.request_cpu, self.request_memory)
    }
}
...
fn print_result(context: &str, info: &[Option<PrintArguments>]) {
    ...
    info.into_iter().flatten().for_each(|item| println!("{}", item))
}
...
fn collect_info(p: Pod) -> Option<PrintArguments> {
    ...
    let containers: Vec<&ResourceRequirements> = spec.containers.iter()
